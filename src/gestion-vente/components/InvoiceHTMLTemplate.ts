/**
 * Template HTML pour la génération de factures
 * Ce template est utilisé pour créer une facture HTML qui sera ensuite convertie en PDF
 */
...
export const generateInvoiceHTML = (data: any): string => {
  ...
  if (warehouse?.logo) {
    ...
    console.log("Logo original:", warehouse.logo);
    console.log("Logo path nettoyé:", logoPath);
    ...
    console.log("Aucun logo d'entrepôt trouvé dans les données:", warehouse);
  ...
  const paymentsHTML = (order.payments || [])
    .map(
      ...
      <span style="display:block; margin-top: 5px; padding: 5px; background-color: ${secondaryColor}; border-radius: 4px;">
        ${index + 1}. ${
        payment.date
          ? new Date(payment.date).toLocaleDateString("fr-FR")
          : "N/A"
      } - <strong>${
        payment.payment_mode_name || "Mode non spécifié"
      }</strong> - ${formatNumber(
        payment.amount || payment.montant_affecte || 0
      )} CFA
      </span>
    `
    )
  ...
  let statusColor = dangerColor; // Rouge par défaut (non payé)
  let statusText = "Non payé";
  let statusBgColor = "#FEE2E2"; // Fond léger rouge

  if (
    paymentStatus &&
    (paymentStatus.toLowerCase() === "paid" ||
      paymentStatus.toLowerCase() === "payé")
  ) {
    statusColor = successColor; // Vert pour payé
    statusText = "Payé";
    statusBgColor = "#DEF7EC"; // Fond léger vert
  } else if (
    paymentStatus &&
    (paymentStatus.toLowerCase().includes("partial") ||
      paymentStatus.toLowerCase().includes("partiel"))
  ) {
    statusColor = warningColor; // Orange pour partiellement payé
    statusText = "Partiellement payé";
    statusBgColor = "#FEF3C7"; // Fond léger orange
  }
          ...
          <div class="payment-info">
            ...
            <div style="margin-top: 15px; margin-bottom: 5px;">
              ...
              <div class="detail-info">
                ...
                <span class="detail-value">${formatNumber(montantDu)} CFA</span>
              </div>
            </div>
            <div style="margin-top: 15px;" class="section-title">
              Détails de Paiement:
            </div>
            <div style="margin-top: 5px;">
              ${
                paymentsHTML ||
                "<span style='color: #a0aec0;'>Aucun paiement enregistré</span>"
              }
            </div>
          </div>
          ...
          <div class="terms">
            ...
            <div style="margin-top: 5px;">
              ${
                ...
                "<span style='color: #a0aec0;'>Aucun terme spécifié</span>"
              }
            </div>
          </div>
